...
server_host     = "0.0.0.0"   # ip for preview server eg. 0.0.0.0:4000
server_port     = "4000"      # port for preview server eg. 0.0.0.0:4000
...
task :preview do
  ...
  jekyllPid = Process.spawn({"OCTOPRESS_ENV"=>"preview"}, "jekyll build --watch")
  compassPid = Process.spawn("compass watch")
  rackupPid = Process.spawn("rackup --port #{server_port} --host #{server_host}")
